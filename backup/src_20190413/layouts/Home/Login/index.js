...
import '../style.css';
import './style.css';
import logo from '../../../assets/img/SkearLogo2.png';
  ...
  handleSubmit = (e) => {
    ...
    this.props.form.validateFields((err, values) => {
      if (err) {
        ...
        if(values.userName === process.env.REACT_APP_BEAR_ID && values.password === process.env.REACT_APP_BEAR_MDP ){
          localStorage.setItem("username", "Bear");
          localStorage.setItem("email", "bear@gmail.com");
          localStorage.setItem("id", "cjsk6gf9bbbhv0c15rfkb6dxg");
          localStorage.setItem("connect", true);
          window.location.reload();
        } else if (values.userName === process.env.REACT_APP_SATAN_ID && values.password === process.env.REACT_APP_SATAN_MDP ) {
          localStorage.setItem("username", "Satan");
          localStorage.setItem("email", "aure@gmail.com");
          localStorage.setItem("id", "cjsk6hig0bbr80c1519t9xexi");
          localStorage.setItem("connect", true);
          window.location.reload();
        } else if (values.userName === process.env.REACT_APP_GUIGUI_ID && values.password === process.env.REACT_APP_GUIGUI_MDP ) {
          localStorage.setItem("username", "Guigui");
          localStorage.setItem("email", "guigui@gmail.com");
          localStorage.setItem("id", "cjsk6kjxxbcco0c152t0tnra7");
          localStorage.setItem("connect", true);
          window.location.reload();
        } else if (values.userName === process.env.REACT_APP_PONEY_ID && values.password === process.env.REACT_APP_PONEY_MDP ) {
          localStorage.setItem("username", "Poney");
          localStorage.setItem("email", "poney@gmail.com");
          localStorage.setItem("id", "cjsk6lxi5bcxb0c1561q6v2xx");
          localStorage.setItem("connect", true);
          window.location.reload();
        } else if (values.userName === process.env.REACT_APP_BELZ_ID && values.password === process.env.REACT_APP_BELZ_MDP ) {
          localStorage.setItem("username", "Belz");
          localStorage.setItem("email", "balz@gmail.com");
          localStorage.setItem("id", "cjsk6mzoubd3f0c15kfnp0oo0");
          localStorage.setItem("connect", true);
          window.location.reload();
      ...
      <Form onSubmit={this.handleSubmit} style={{ margin: 'auto' }} className="login-form">
          ...
          <FormItem>
            {getFieldDecorator('remember', {
              ...
              <Checkbox style={{ color: 'rgba(255, 255, 255, 0.88)' }}>Remember me</Checkbox>
            )}
